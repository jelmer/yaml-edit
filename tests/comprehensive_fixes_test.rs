...
fn test_single_quote_backslash_is_literal() {
    ...
    // The string 'it\'s literal' is parsed as 'it\' (the quote ends at \')
    // and the remaining "s literal'" is malformed content that is preserved for lossless parsing
    let yaml = r#"literal: 'it\'s literal'"#;

    let parsed = Parse::parse_yaml(yaml);
    // This is actually malformed YAML since the string ends at \' and "s literal'" is extra
    // But our lossless parser preserves all content

    // For a lossless parser, value.text() returns all content in the value node,
    // including any malformed trailing content
    let tree = parsed.tree();
    if let Some(doc) = tree.document() {
        if let Some(mapping) = doc.as_mapping() {
            if let Some(value) = mapping.get("literal") {
                let text = value.text().to_string();
                // The lossless parser preserves the entire value including malformed parts
                assert_eq!(
                    text, "'it\\'s literal'",
                    "Lossless parser should preserve entire value content"
                );
            }
        }
    }
