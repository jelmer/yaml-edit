use yaml_edit::{Yaml, Scalar, Mapping};
use std::str::FromStr;
...
fn test_invalid_colon_in_plain_scalar() {
    ...
    match parsed {
        Ok(yaml) => {
            ...
            if let Some(doc) = yaml.document() {
                if let Some(mapping) = doc.as_mapping() {
                    ...
                    assert!(mapping.keys().any(|k| k == "not_url_2"), 
        ...
        Err(e) => {
            ...
            assert!(error_str.contains("mapping") || error_str.contains("colon") || 
...
fn test_multiple_colons_in_value() {
    ...
    for (name, yaml, should_be_valid) in test_cases {
        ...
        if should_be_valid {
            assert!(parsed.is_ok(), 
                    "{}: '{}' should parse successfully", name, yaml);
                ...
                if let Some(doc) = yaml_doc.document() {
                    if let Some(mapping) = doc.as_mapping() {
                        assert_eq!(mapping.keys().count(), 1, 
                                  "{}: Should have exactly 1 key", name);
                    }
                }
            }
        } else {
        ...
        Ok(yaml_doc) => {
            if let Some(doc) = yaml_doc.document() {
                if let Some(mapping) = doc.as_mapping() {
                    ...
                    assert!(has_good_key || has_another_good, 
